...
class board {
   ...
   public:
    ...
    void popup() {
        ...
        if (num) set(space[rand() % num], rand() % 10 ? 1 : 2);
    }
...
class pattern : public feature {
   ...
   public:
    ...
    virtual float estimate(const board &b) const {
        ...
        for (int i = 0; i < iso_last; i++) {
            // 取得board以第i種結構組成的index
            size_t index = indexof(isomorphic[i], b);
            // operator會回傳該index的pattern數值
   ...
   protected:
    size_t indexof(const std::vector<int> &patt, const board &b) const {
        // TODO
        size_t index = 0;
        // 將board數值轉換成value，第16格在最高位，第一個在最低位
   ...
   public:
    ...
    state select_best_move(const board &b) const {
        ...
        for (state *move = after; move != after + 4; move++) {
            if (move->assign(b)) {
                ...
                for (int i = 0; i < 16; i++) {
                    // 2和4出現的比例9:1，因此出現2與出現4的board值以9:1的加權累計
                    if (now.at(i) == 0) {
                        cnt++;
                        now.set(i, 1);
                ...
                // cnt=0代表沒空格，因此不必計算
                if (cnt != 0) {
                    // 因board可能有多個空格，用cnt去紀錄有幾個空格，全部累計後再取平均
    ...
    void update_episode(std::vector<state> &path, float alpha = 0.1) const {
        ...
        for (path.pop_back(); path.size(); path.pop_back()) {
            ...
            // 計算差值
            float err =
                target - (estimate(move.before_state()) - move.reward());
            // 更新差值*比例到weight
