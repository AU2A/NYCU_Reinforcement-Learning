...
class AtariNet(nn.Module):
    ...
    def forward(self, x, eval=True, ac_train_batch=[]):
        x = x.float() / 255.
        x = self.cnn(x)
        ...
        if len(ac_train_batch) == 0:
            logp = dist.log_prob(action)
        else:
            ac_train_batch = torch.squeeze(ac_train_batch)
            logp = dist.log_prob(ac_train_batch)

        entropy = dist.entropy().mean()

        logp = torch.squeeze(logp)
