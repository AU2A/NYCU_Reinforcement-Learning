...
def get_img_views():
    ...
    lap = int(info["lap"])
    score = lap + progress - 1.0

    # Get the images
    img1 = env.env.force_render(
        render_mode="rgb_array_higher_birds_eye",
        width=540,
        height=540,
        position=np.array([4.89, -9.30, -3.42]),
        fov=120,
    )
    img2 = env.env.force_render(
        render_mode="rgb_array_birds_eye", width=270, height=270
    )
